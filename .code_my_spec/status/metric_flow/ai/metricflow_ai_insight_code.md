<!-- cms:task type="ComponentCode" component="MetricFlow.Ai.Insight" -->
...
Component Description: Ecto schema storing an AI-generated insight derived from correlation analysis. Each record represents one actionable recommendation produced by the InsightsGenerator for a given account, optionally linked to the specific CorrelationResult that motivated it. Insights carry a suggestion_type enum that categorises the recommended action, a confidence float scored between 0.0 and 1.0, and a metadata map for supplementary context such as metric names and correlation values. Belongs to Account and optionally belongs to CorrelationResult. Has many SuggestionFeedback records that capture user ratings.
Type: schema

Spec File: .code_my_spec/spec/metric_flow/ai/insight.spec.md
Test File: test/metric_flow/ai/insight_test.exs
...
Write the implementation to lib/metric_flow/ai/insight.ex
