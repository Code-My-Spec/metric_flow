<!-- cms:task type="ComponentCode" component="MetricFlow.Ai.ChatSession" -->
...
Component Name: ChatSession
Component Description: Ecto schema representing a conversational AI chat session. Each session belongs to a user and an account, and may optionally be scoped to a specific correlation result, metric, or dashboard so the assistant has focused context for its responses. Sessions begin in the :active state and can be moved to :archived by the user; there is no transition back to :active. Has many ChatMessages ordered by insertion time ascending.
Type: schema

Spec File: .code_my_spec/spec/metric_flow/ai/chat_session.spec.md
Test File: test/metric_flow/ai/chat_session_test.exs
...
Write the implementation to lib/metric_flow/ai/chat_session.ex
