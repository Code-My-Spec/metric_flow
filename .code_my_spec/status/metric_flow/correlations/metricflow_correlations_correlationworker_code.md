<!-- cms:task type="ComponentCode" component="MetricFlow.Correlations.CorrelationWorker" -->
...
Component Name: CorrelationWorker
Component Description: Oban worker that executes correlation calculations. Receives job_id and account_id in args. Updates CorrelationJob status to running, queries metric time series from MetricFlow.Metrics, computes Pearson correlations with TLCC across 0-30 day lags using the Math module, persists CorrelationResult records, and updates job status to completed or failed. Uses Task.async_stream for parallel pair-wise computation.
Type: module

Spec File: .code_my_spec/spec/metric_flow/correlations/correlation_worker.spec.md
Test File: test/metric_flow/correlations/correlation_worker_test.exs
...
Write the implementation to lib/metric_flow/correlations/correlation_worker.ex
