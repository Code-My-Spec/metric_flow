<!-- cms:task type="ComponentCode" component="MetricFlow.DataSync.SyncWorker" -->
...
Component Name: SyncWorker
Component Description: Oban worker that executes data sync operations. Receives integration_id and user_id in args. Updates SyncJob status to running, retrieves integration tokens, delegates to appropriate DataProvider based on provider, persists metrics via MetricFlow.Metrics, creates SyncHistory record with results, and updates SyncJob status to completed or failed. Handles token refresh when tokens are expired.
Type: module

Spec File: .code_my_spec/spec/metric_flow/data_sync/sync_worker.spec.md
Test File: test/metric_flow/data_sync/sync_worker_test.exs
...
Write the implementation to lib/metric_flow/data_sync/sync_worker.ex
