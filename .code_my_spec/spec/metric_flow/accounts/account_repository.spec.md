...
Creates a new team account and adds the calling user as the owner (originator). Validates required fields and slug uniqueness. Executes atomically within a transaction.
...
7. Broadcast {:created, account} to the Phoenix.PubSub topic "accounts:user:#{user_id}"
8. Return {:ok, account} with the persisted Account on success, or {:error, changeset} on validation failure

**Test Assertions**:
- creates a team account with valid attrs
- adds the calling user as the owner member
...
- broadcasts {:created, account} on success
...
4. Update the account record in the database with Repo.update/1
5. Broadcast {:updated, account} to the Phoenix.PubSub topic "accounts:user:#{user_id}"
6. Return {:ok, account} with the updated Account on success, or {:error, changeset} on validation failure
...
**Process**:
1. Verify the calling user is a member of the account by querying account_members for a matching user_id and account_id (raises if not found)
...
**Process**:
1. Query account_members for the record matching the given user_id and account_id
...
4. If the target member currently has role :owner, query the count of owners for the account — return {:error, :last_owner} if only one owner exists
5. Apply AccountMember.changeset/2 with the new role
6. Update the record in the database with Repo.update/1
7. Broadcast {:updated, account_member} to the Phoenix.PubSub topic "account_members:user:#{user_id}"
8. Return {:ok, account_member} with the updated AccountMember
...
- returns {:error, :last_owner} when demoting the sole owner
- returns {:error, :unauthorized} for account_manager and read_only roles
- broadcasts {:updated, account_member} on success

### remove_user_from_account/3

Removes a user from an account. Owners and admins may remove members. The last owner of an account cannot be removed.
...
2. Return {:error, :unauthorized} if the check fails
3. Fetch the AccountMember record for the target user_id and account_id
4. If the target member has role :owner, query the count of owners for the account — return {:error, :last_owner} if only one owner exists
5. Delete the AccountMember record with Repo.delete/1
6. Broadcast {:deleted, account_member} to the Phoenix.PubSub topic "account_members:user:#{user_id}"
7. Return {:ok, account_member} with the deleted AccountMember struct

**Test Assertions**:
- removes a member when called by the owner
- removes a member when called by an admin
- returns {:error, :last_owner} when removing the sole owner
- returns {:error, :unauthorized} for account_manager and read_only roles
- broadcasts {:deleted, account_member} on success
...
2. Return {:error, :unauthorized} if the check fails
3. Check whether an AccountMember record already exists for the given user_id and account_id — return {:error, :already_member} if so
4. Build an AccountMember changeset with user_id, account_id, and role
5. Insert the AccountMember record with Repo.insert/1
6. Broadcast {:created, account_member} to the Phoenix.PubSub topic "account_members:user:#{user_id}"
7. Return {:ok, account_member} with the inserted AccountMember

**Test Assertions**:
- adds a member with the given role
- returns {:error, :already_member} when the user is already a member
- allows owner to add a member with owner role
- prevents admin from adding a member with owner role
- returns {:error, :unauthorized} for account_manager and read_only roles
- broadcasts {:created, account_member} on success
