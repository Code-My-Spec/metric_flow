# MetricFlow.Integrations.Providers.Anthropic

Anthropic OAuth provider implementation using Assent.Strategy.OAuth2. Configures OAuth with Anthropic credentials and the "read" scope. Normalizes Anthropic user data to the application domain model.
...
Returns Assent strategy configuration for Anthropic OAuth with required credentials, redirect URI, and authorization parameters.
...
**Process**:
1. Fetch anthropic_client_id from application config
2. Fetch anthropic_client_secret from application config
3. Build redirect URI by concatenating Endpoint.url() with "/integrations/oauth/callback/anthropic"
...
- includes redirect_uri with "/integrations/oauth/callback/anthropic" path
- includes authorization_params with read scope

### strategy/0

Returns the Assent.Strategy.OAuth2 module for the Anthropic OAuth flow.
...
Transforms Anthropic user data into the application domain model with provider_user_id, email, name, username, and avatar_url.
