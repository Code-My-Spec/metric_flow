...
**Description**: As a client user, I want my Google Ads account data to be synced daily so that advertising spend and performance metrics are available alongside my other platform data for correlation analysis.
**Priority**: 18
...
### Criterion 4762: System fetches Google Ads data using the Google Ads API via the google-ads-api client library, querying the 'customer' entity with date segments

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4762_system_fetches_google_ads_data_using_the_google_ads_api_via_the_google-ads-api_client_library_querying_the_customer_entity_with_date_segments_spex.exs`
...
### Criterion 4763: Data is fetched per Google Ads customer ID (googleAdsPropertyId) configured for each account

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4763_data_is_fetched_per_google_ads_customer_id_googleadspropertyid_configured_for_each_account_spex.exs`
...
### Criterion 4764: System requires a valid GOOGLE_DEVELOPER_TOKEN environment variable and a manager account ID (GOOGLE_MANAGER_ACCOUNT_ID) to authenticate API calls

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4764_system_requires_a_valid_google_developer_token_environment_variable_and_a_manager_account_id_google_manager_account_id_to_authenticate_api_calls_spex.exs`
...
### Criterion 4765: OAuth refresh token from the stored Google token set is used per request — no separate Google Ads OAuth flow

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4765_oauth_refresh_token_from_the_stored_google_token_set_is_used_per_request_no_separate_google_ads_oauth_flow_spex.exs`
...
### Criterion 4766: System syncs the following metrics as core daily values: clicks, impressions, cost_micros (converted to cost by dividing by 1,000,000), all_conversions, conversions

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4766_system_syncs_the_following_metrics_as_core_daily_values_clicks_impressions_cost_micros_converted_to_cost_by_dividing_by_1000000_all_conversions_conversions_spex.exs`
...
### Criterion 4767: Metrics are fetched at account level only (customer entity, no campaign or ad group segmentation) — one row per day per metric

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4767_metrics_are_fetched_at_account_level_only_customer_entity_no_campaign_or_ad_group_segmentation_one_row_per_day_per_metric_spex.exs`
...
### Criterion 4768: Data fetched is segmented by date only — no additional dimensions (e.g., campaign, device, network) are stored at this stage; adding dimensions would require schema changes and re-aggregation logic before correlation can be run

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4768_data_fetched_is_segmented_by_date_only_no_additional_dimensions_eg_campaign_device_network_are_stored_at_this_stage_adding_dimensions_would_require_schema_changes_and_re-aggregation_logic_before_correlation_can_be_run_spex.exs`
...
### Criterion 4769: On first sync, system backfills up to 548 days of historical data; subsequent syncs fetch from the day after the last stored metric date

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4769_on_first_sync_system_backfills_up_to_548_days_of_historical_data_subsequent_syncs_fetch_from_the_day_after_the_last_stored_metric_date_spex.exs`
...
### Criterion 4770: Sync uses retryWithBackoff with up to 3 retries and 2-second initial delay for transient API errors

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4770_sync_uses_retrywithbackoff_with_up_to_3_retries_and_2-second_initial_delay_for_transient_api_errors_spex.exs`
...
### Criterion 4771: Google Ads API errors are extracted from error.response.errors[0] and surfaced with full context (customerId, dateRange, errorCode)

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4771_google_ads_api_errors_are_extracted_from_errorresponseerrors0_and_surfaced_with_full_context_customerid_daterange_errorcode_spex.exs`
...
### Criterion 4772: Sync failures are logged with full error details and surfaced in Sync Status and History

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4772_sync_failures_are_logged_with_full_error_details_and_surfaced_in_sync_status_and_history_spex.exs`
...
### Criterion 4773: cost_micros is always divided by 1,000,000 before storage so all cost values are in standard currency units

**Status**: ❌ MISSING
**Expected File Path**: `test/spex/510_sync_google_ads_data/criterion_4773_cost_micros_is_always_divided_by_1000000_before_storage_so_all_cost_values_are_in_standard_currency_units_spex.exs`
