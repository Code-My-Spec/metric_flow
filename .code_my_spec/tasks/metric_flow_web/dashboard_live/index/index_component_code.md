...
Component Description: List dashboards available to the authenticated user. Shows both the user's own saved dashboards and system-provided canned dashboards. Unauthenticated requests are redirected to `/users/log-in` by the router's `:require_authenticated_user` pipeline.
Type: liveview

Spec File: .code_my_spec/spec/metric_flow_web/dashboard_live/index.spec.md
Test File: test/metric_flow_web/live/dashboard_live/index_test.exs
...
Write the implementation to lib/metric_flow_web/live/dashboard_live/index.ex
