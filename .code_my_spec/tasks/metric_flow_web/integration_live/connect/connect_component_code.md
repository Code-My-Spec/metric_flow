<!-- cms:task type="ComponentCode" component="MetricFlowWeb.IntegrationLive.Connect" -->
...
Component Description: OAuth connection flow for linking marketing platforms to a user account. Displays all supported platforms (Google Ads, Facebook Ads, Google Analytics) with their current connection status and per-platform OAuth initiation links. Also serves as the OAuth callback handler — when the user returns from the provider, the view processes the authorization code, persists the integration, and shows a confirmation or error message.
Type: liveview

Spec File: .code_my_spec/spec/metric_flow_web/integration_live/connect.spec.md
Test File: test/metric_flow_web/live/integration_live/connect_test.exs
...
Write the implementation to lib/metric_flow_web/live/integration_live/connect.ex
