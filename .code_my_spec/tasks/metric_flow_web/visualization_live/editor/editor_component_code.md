...
Component Description: Create or edit an individual standalone visualization. Authenticated users can build a named Vega-Lite chart from available metrics, preview it, and save it to their library. The resulting visualization may later be added to any dashboard via the Dashboard editor. Unauthenticated requests are redirected to `/users/log-in` by the router's `:require_authenticated_user` pipeline.
Type: liveview

Spec File: .code_my_spec/spec/metric_flow_web/visualization_live/editor.spec.md
Test File: test/metric_flow_web/live/visualization_live/editor_test.exs
...
Write the implementation to lib/metric_flow_web/live/visualization_live/editor.ex
