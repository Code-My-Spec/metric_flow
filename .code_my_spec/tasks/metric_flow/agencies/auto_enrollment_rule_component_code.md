...
Component Description: Ecto schema representing domain-based auto-enrollment configuration for agency accounts. Stores email domain patterns, enabled status, and default access level for auto-enrolled users. Enforces one rule per agency via unique constraint. Provides changeset validation for domain format and access level values.
Type: module

Spec File: .code_my_spec/spec/metric_flow/agencies/auto_enrollment_rule.spec.md
Test File: test/metric_flow/agencies/auto_enrollment_rule_test.exs
...
Write the implementation to lib/metric_flow/agencies/auto_enrollment_rule.ex
