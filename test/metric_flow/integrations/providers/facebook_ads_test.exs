defmodule MetricFlow.Integrations.Providers.FacebookAdsTest do
  use ExUnit.Case, async: true

  alias MetricFlow.Integrations.Providers.FacebookAds

  # ---------------------------------------------------------------------------
  # Fixtures
  # ---------------------------------------------------------------------------

  defp valid_user_data do
    %{
      "sub" => "10229461234567890",
      "email" => "jane@example.com",
      "name" => "Jane Doe",
      "login" => "janedoe",
      "picture" => "https://graph.facebook.com/123/picture"
    }
  end

  defp user_data_with_id do
    %{
      "id" => "10229461234567890",
      "email" => "jane@example.com",
      "name" => "Jane Doe"
    }
  end

  # ---------------------------------------------------------------------------
  # config/0
  # ---------------------------------------------------------------------------

  describe "config/0" do
    test "returns keyword list with required OAuth configuration keys" do
      config = FacebookAds.config()
      assert Keyword.has_key?(config, :client_id)
      assert Keyword.has_key?(config, :client_secret)
      assert Keyword.has_key?(config, :redirect_uri)
      assert Keyword.has_key?(config, :authorization_params)
    end

    test "includes client_id from application config" do
      config = FacebookAds.config()
      assert is_binary(Keyword.get(config, :client_id))
    end

    test "includes client_secret from application config" do
      config = FacebookAds.config()
      assert is_binary(Keyword.get(config, :client_secret))
    end

    test "includes redirect_uri with /integrations/oauth/callback/facebook_ads path" do
      config = FacebookAds.config()
      assert String.ends_with?(Keyword.get(config, :redirect_uri), "/app/integrations/oauth/callback/facebook_ads")
    end

    test "includes authorization_params with read scope" do
      config = FacebookAds.config()
      params = Keyword.get(config, :authorization_params)
      assert Keyword.get(params, :scope) == "read"
    end
  end

  # ---------------------------------------------------------------------------
  # strategy/0
  # ---------------------------------------------------------------------------

  describe "strategy/0" do
    test "returns Assent.Strategy.OAuth2 module" do
      assert FacebookAds.strategy() == Assent.Strategy.OAuth2
    end
  end

  # ---------------------------------------------------------------------------
  # normalize_user/1
  # ---------------------------------------------------------------------------

  describe "normalize_user/1" do
    test "returns ok tuple with normalized user data for valid input" do
      assert {:ok, normalized} = FacebookAds.normalize_user(valid_user_data())
      assert is_map(normalized)
    end

    test "extracts provider_user_id from sub field" do
      {:ok, normalized} = FacebookAds.normalize_user(valid_user_data())
      assert normalized.provider_user_id == "10229461234567890"
    end

    test "falls back to id field for provider_user_id" do
      {:ok, normalized} = FacebookAds.normalize_user(user_data_with_id())
      assert normalized.provider_user_id == "10229461234567890"
    end

    test "extracts email from email field" do
      {:ok, normalized} = FacebookAds.normalize_user(valid_user_data())
      assert normalized.email == "jane@example.com"
    end

    test "extracts avatar_url from picture field" do
      {:ok, normalized} = FacebookAds.normalize_user(valid_user_data())
      assert normalized.avatar_url == "https://graph.facebook.com/123/picture"
    end

    test "returns error :invalid_user_data when input is not a map" do
      assert {:error, :invalid_user_data} = FacebookAds.normalize_user("not a map")
      assert {:error, :invalid_user_data} = FacebookAds.normalize_user(nil)
    end
  end
end
