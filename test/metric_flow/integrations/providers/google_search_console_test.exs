defmodule MetricFlow.Integrations.Providers.GoogleSearchConsoleTest do
  ...
  describe "config/0" do
    ...
    test "returns keyword list with required OAuth configuration keys" do
      ...
      capture_log(fn ->
        config = GoogleSearchConsole.config()
    ...
    test "includes redirect_uri pointing to the google_search_console callback path" do
      if not has_google_credentials?(), do: flunk("Google OAuth credentials not configured in .env.test")
    ...
    test "includes authorization_params with email, profile, and webmasters.readonly scopes" do
      if not has_google_credentials?(), do: flunk("Google OAuth credentials not configured in .env.test")
      ...
      capture_log(fn ->
        ...
        assert String.contains?(scope, "webmasters.readonly")
      end)
    end
    ...
    test "raises ArgumentError when google_client_id is not configured" do
      ...
      assert_raise ArgumentError, fn ->
        capture_log(fn -> GoogleSearchConsole.config() end)
      end
    end
  ...
  describe "strategy/0" do
    test "returns Assent.Strategy.Google module" do
      assert GoogleSearchConsole.strategy() == Assent.Strategy.Google
    end
  end
  ...
  describe "normalize_user/1" do
    test "returns ok tuple with normalized user data for valid Google user data" do
      assert {:ok, normalized} = GoogleSearchConsole.normalize_user(valid_user_data())
    ...
    test "extracts hosted_domain from \"hd\" field for Google Workspace accounts" do
      assert {:ok, normalized} = GoogleSearchConsole.normalize_user(workspace_user_data())
    ...
    test "handles missing optional fields (name, picture, hd) gracefully with nil" do
      assert {:ok, normalized} = GoogleSearchConsole.normalize_user(minimal_user_data())
    ...
    test "returns error :missing_provider_user_id when \"sub\" is missing or nil" do
      ...
      assert {:error, :missing_provider_user_id} = GoogleSearchConsole.normalize_user(missing_sub)
      assert {:error, :missing_provider_user_id} = GoogleSearchConsole.normalize_user(nil_sub)
    end

    test "returns error :invalid_user_data when input is not a map" do
      assert {:error, :invalid_user_data} = GoogleSearchConsole.normalize_user("not a map")
      assert {:error, :invalid_user_data} = GoogleSearchConsole.normalize_user(42)
      assert {:error, :invalid_user_data} = GoogleSearchConsole.normalize_user([:list])
    end
    ...
    test "handles personal Google accounts without hosted domain" do
      assert {:ok, normalized} = GoogleSearchConsole.normalize_user(personal_account_user_data())
