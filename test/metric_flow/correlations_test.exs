defmodule MetricFlow.CorrelationsTest do
  ...
  describe "run_correlations/2" do
    test "returns ok tuple with correlation job when valid" do
    ...
    test "creates job with status :pending" do
      {user, scope} = user_with_scope()
      insert_two_metrics_for_sufficient_data!(user.id)
    ...
    test "returns error :insufficient_data when fewer than 30 days of metric data" do
      {_user, scope} = user_with_scope()
    ...
    test "returns error :already_running when a job is already in progress" do
      {user, scope} = user_with_scope()
      insert_two_metrics_for_sufficient_data!(user.id)
  ...
  describe "schedule_daily_correlations/0" do
    test "schedules jobs for users with sufficient data" do
    ...
    test "skips users with fewer than 30 days of metric data" do
      {_user, _scope} = user_with_scope()
    ...
    test "skips users who already have recent correlation results (within 24 hours)" do
      {user, scope} = user_with_scope()
      integration_fixture(user)
      insert_metric!(user.id, %{metric_name: "sessions", value: 10.0})
      insert_metric!(user.id, %{metric_name: "revenue", value: 500.0})
  ...
  describe "get_latest_correlation_summary/1" do
    test "returns summary with results sorted by absolute coefficient" do
      {_user, scope} = user_with_scope()
      job = insert_completed_correlation_job!(scope)

      insert_correlation_result!(scope, job, %{metric_name: "weak_m", coefficient: 0.2})
      insert_correlation_result!(scope, job, %{metric_name: "strong_m", coefficient: 0.9})
      insert_correlation_result!(scope, job, %{metric_name: "moderate_m", coefficient: 0.5})

      summary = Correlations.get_latest_correlation_summary(scope)

      coefficients = Enum.map(summary.results, &abs(&1.coefficient))
      assert coefficients == Enum.sort(coefficients, :desc)
      assert hd(summary.results).metric_name == "strong_m"
    end

    test "includes last_calculated_at timestamp from the job" do
      {_user, scope} = user_with_scope()
      completed_at = DateTime.utc_now() |> DateTime.truncate(:microsecond)
    ...
    test "includes data_window date range" do
      {_user, scope} = user_with_scope()
    ...
    test "includes data_points_count" do
      {_user, scope} = user_with_scope()
    ...
    test "returns empty results list with no_data flag when no correlations exist" do
      {_user, scope} = user_with_scope()
    ...
    test "includes goal metric information" do
      {_user, scope} = user_with_scope()
